...
$sql="select * from collect where kind=1 and username='".$username."' and number='".$number."'";
...
if(mysql_num_rows($result)>0)
{
	...
	if ($r!=true)
	{
		$sql="delete from collect where kind=1 and username='".$username."' and number='".$number."'";
		$result=mysql_query($sql);
		if($result)
		{
...
else
{
	...
	$img=$row['articleimg']; 
	$collect=$row['collect']+1; 
 	$sql="insert into collect (username,number,name,img,kind,tjrname) values('".$username."','".$number."','".$name."','".$img."','1','管理员')";
	$result=mysql_query($sql);
 	if($result)
 	{
		$sql="update adarticle set collect='".$collect."' where articleid='".$number."'";
