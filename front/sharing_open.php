...
.pop {  display: none;  width: 600px; min-height: 200px;  max-height: 300px;  height:470px;  position: absolute;  top: 0;  left: 0;  bottom: 0;  right: 0;  margin: auto;  padding: 25px;  z-index: 130;  border-radius: 8px;  background-color: #fff;  box-shadow: 0 3px 18px rgba(100, 0, 0, .5);  }
...
.pop-cancel, .pop-ok { font-family:"微软雅黑";height: 35px;  padding:8px 15px;  margin:15px 5px;  border: none;  border-radius: 5px;  background-color: #337AB7;  color: #fff;  cursor:pointer;  }
	...
	<div class="main-left">
        <div class="main-left2">
        	...
        	<div class="main-left2-1">
            	<img src="../images/userwenzhang/<?php echo $row['articleimg'];?>" ><!--图片-->
            </div>
...
<div align="center" id="wall" style="padding-top: 20px; font-family: '微软雅黑'; font-size: small;">
	<div class="container">
		...
		$sql="select * from comment where articleid=$articleid and kind=0 order by dzlike desc";
...
if(isset($_POST['pl']))
{
	...
	$sql="select *from comment where coment='".$comment."' and kind=0 and articleid='".$articleid."'";
	$result=mysql_query($sql);
	...
	else
	{
		...
		$row=mysql_fetch_array($result);
		$image=$row['image'];
	 	$sql="insert into comment (reviewer,respondent,coment,image,time,articleid,kind) values('".$username."','".$tjrname."','".$comment."','".$image."','".$day."','".$articleid."','0')";
	 	$result=mysql_query($sql);
	 	if($result)
	 	{
