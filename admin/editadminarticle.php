  ...
  <h3><a href="javascript:history.go(-1)" class="actionBtn">文章列表</a>编辑文章</h3>
	...
	<p class="weixinAudio"  style="float:left;">
      ...
      <tr>
       ...
       <td>
        <input type="text" name="musicdescribes" value="<?php echo $row['musicdescribes'];?>" size="50" class="inpMain" />
       </td>
      </tr>      
      <tr>
...
<?php	
	if(isset($_POST['submit']))
	{
		...
		$sql="update adarticle set name='".$name."',musicdescribes='".$musicdescribes."',classes='".$classes."',articledescribes='".$articledescribes."',time='".$day."',collect='".$collect."' where articleid='".$id."'";
	 	$result=mysql_query($sql);
	 	if($result)
	 	{
